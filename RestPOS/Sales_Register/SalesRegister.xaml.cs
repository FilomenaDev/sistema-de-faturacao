    ...
    public partial class SalesRegister : Window
    {
        ...
        private void btnholdsale_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    Hold_sales_item(DateTime.Now.ToString("dd-MM-yyyy").ToString());
                    //// TokennoInsert
                    tokennoInsert(); 
                    tokennumberget();
        ...
        {
            string orderdatetime = DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss");
        ...
        private void btnDirectSale_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    payment_item(lblTotalPayable.Text, "0", "0", "Cash", DateTime.Now.ToString("dd-MM-yyyy").ToString(), "00000000", "Cosumidor Final");

                    ///// save sales items one by one  
                    sales_item(DateTime.Now.ToString("dd-MM-yyyy").ToString());
        ...
        {            
            ...
            txtCustName.Text = "Consumidor Final";
        }
        ...
        public void payment_item(string payamount, string changeamount, string dueamount, string salestype, string salesdate, string custid, string Comment)
        {
          ...
          //  string due = txtDueAmount.Text;
           ...
           // string Commen = txtCustName.Text + " " + txtcomment.Text;
            ...
            else
            {
                try
                {
                    if (Convert.ToDouble(txtPaidAmount.Text) >= Convert.ToDouble(lblTotalPayable.Text))
                    {
                        ...
                        // changeAmt = Math.Round(changeAmt, 2); modificado
        ...
        private void txtDiscountRate_TextChanged(object sender, TextChangedEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    // double totaldiscount = (subtotal * Discountvalue) / 100; //Counter discount  // 95 * 5 /100 = 4.75  modificado
                    ...
                    //double disPlusOverallDiscount = totaldiscount + Convert.ToDouble(lblTotalDisCount.Text); // 4.75 + 5 = 9.75modificado
                    // disPlusOverallDiscount = Math.Round(disPlusOverallDiscount, 2);modificado
                    // lbloveralldiscount.Text = disPlusOverallDiscount.ToString();  // Overall discount 9.75modificado

                    //double subtotalafteroveralldiscount = subtotal - totaldiscount; // 95 - 4.75 = 90.25modificado
                    // subtotalafteroveralldiscount = Math.Round(subtotalafteroveralldiscount, 2); modificado
                    lblsubtotal.Text = subtotal.ToString();

                    double payable = subtotal + Convert.ToDouble(lblTotalVAT.Text);
                    //  payable = Math.Round(payable, 2); modificado
                    lblTotalPayable.Text = payable.ToString();
                    lblTotalPayableTabpayment.Text = payable.ToString();
                    btnCurrentAmount.Content = payable.ToString();
