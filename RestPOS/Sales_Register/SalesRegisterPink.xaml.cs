    ...
    public partial class SalesRegisterPink : Window
    {
       ...
       // String holdtableno = "";
        ...
        public SalesRegisterPink()
        {
            ...
            t.Columns.Add(new DataColumn("Preço"));
            t.Columns.Add(new DataColumn("Qty"));
            t.Columns.Add(new DataColumn("Total"));
            t.Columns.Add(new DataColumn("ID"));
            t.Columns.Add(new DataColumn("Total Dis"));
            t.Columns.Add(new DataColumn("Total Taxa"));
            t.Columns.Add(new DataColumn("Dis"));
            t.Columns.Add(new DataColumn("Taxa"));
            t.Columns.Add(new DataColumn("KD"));
            t.Columns.Add(new DataColumn("Opções"));
        ...
        public void ResulationHW()
        {
            ...
            {
                double W = (System.Windows.SystemParameters.PrimaryScreenWidth * 15) / 100;
        ...
        public void addtocartitem(string product_id, double itemqty, string options)
        {
            ...
            string sql = "SELECT  product_name as Name , retail_price as Price ,  " + itemqty + "  as QTY, retail_price *  " + itemqty + " as 'Total' ,  " +
                    " (((retail_price * " + itemqty + " ) * discount) / 100) as 'dis amt' , " +
                    " CASE     " +
                    " WHEN taxapply = 1 THEN   (((retail_price * " + itemqty + " )  - (((retail_price * " + itemqty + " ) * discount) / 100))  * " + Taxrate + " ) / 100   " +
                    " ELSE '0.00'  " +
            ...
            else
            {
                ...
                {
                    double DisamtInc = (((Rprice * qty) * disrate) / 100);      // Total Discount amount of this item
                ...
                if (Taxapply != 0)   // If apply  tax 
                {
                    ...
                    double TaxamtInc = ((((Rprice * qty) - (((Rprice * qty) * disrate) / 100)) * Taxrate) / 100);
        ...
        private void txtbarcodescan_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    addtocartitem(txtbarcodescan.Text, 1, "");
                }
                catch
                {
                }
            ...
            try
            {
                ...
                for (i = 0; i < dgrvSalesItemList.SelectedItems.Count; i++)
                {
                    if (Convert.ToDouble(dataRow.Row[2].ToString()) != 1)
                    {
                        ...
                        {
                            double Taxamt = ((((Price * CurrentQty) - (((Price * CurrentQty) * disrate) / 100)) * Taxrate) / 100); // Total Tax amount  of this item
        ...
        private void btnDirectSale_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    payment_item(lblTotalPayable.Text, "0", "0", "Valor Total", DateTime.Now.ToString("yyyy-MM-dd").ToString(), "9999999999", "Consumidor Final");
            ...
            try
            {
                ...
                for (i = 0; i < dgrvSalesItemList.SelectedItems.Count; i++)
                {
                    if (e.Column.SortMemberPath.Equals("Qty"))
                    {
                        ...
                        if (disrate != 0)
                        {
                            double Disamt = (((Price * Qty) * disrate) / 100);      // Total Discount amount of this item
                            dataRow.Row[5] = Disamt;
                        }

                        if (Convert.ToDouble(dataRow.Row[8].ToString()) != 0)
                        {
                            double Taxamt = ((((Price * Qty) - (((Price * Qty) * disrate) / 100)) * Taxrate) / 100); // Total Tax amount  of this item
                            dataRow.Row[6] = Taxamt;
                        }
        ...
        public void payment_item(string payamount, string changeamount, string dueamount, string salestype, string salesdate, string custid, string Comment)
        {
          ...
          //  string due = txtDueAmount.Text;
            ...
            string Commen = txtCustName.Text ;
            ...
            string sql1 = " insert into sales_payment (sales_id, payment_type, payment_amount, change_amount, due_amount, dis, vat, " +
                            ...
                            " '" +  UserInfo.UserName + "','" + Commen + "','POS', '"+ UserInfo.Shopid +"', '" + overalldisRate + "' , '" + vatRate + "', " +
        ...
        private void btnCompleteSalesAndPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    if (MessageBox.Show("Terminar venda?", "Question", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                        ////Save payment info into sales_payment table
            ...
            try
            {
                ...
                else
                {
                    ...
                    // double totaldiscount = Convert.ToDouble(lblTotalDisCount.Text) + Discountvalue;   // Uncomment this line if you want to discount value and comment/delete above line 
                      double disPlusOverallDiscount = totaldiscount + Convert.ToDouble(lblTotalDisCount.Text); // 4.75 + 5 = 9.75modeficado
                      disPlusOverallDiscount = Math.Round(disPlusOverallDiscount, 2);//modeficado
                     lbloveralldiscount.Text = disPlusOverallDiscount.ToString();  // Overall discount 9.75modeficado

                    double subtotalafteroveralldiscount = subtotal - totaldiscount; // 95 - 4.75 = 90.25modeficado
                    subtotalafteroveralldiscount = Math.Round(subtotalafteroveralldiscount, 2);    // modeficado                       
                    lblsubtotal.Text = subtotalafteroveralldiscount.ToString();

                    double payable = subtotalafteroveralldiscount + Convert.ToDouble(lblTotalVAT.Text);//modeficado
                    payable = Math.Round(payable, 2);//modeficado
                    lblTotalPayable.Text = payable.ToString() + Convert.ToDouble(lblTotalVAT.Text);
                    lblTotalPayableTabpayment.Text = payable.ToString() + Convert.ToDouble(lblTotalVAT.Text);
                    btnCurrentAmount.Content = payable.ToString() + Convert.ToDouble(lblTotalVAT.Text);

                    txtPaidAmount.Text = payable.ToString() + Convert.ToDouble(lblTotalVAT.Text);
        ...
        private void Tabstockcontrol_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
